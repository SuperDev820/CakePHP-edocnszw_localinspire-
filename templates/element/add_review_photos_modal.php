		...
		<div class="card-body bg-light">
			...
			<div class="modal-footer photouploader-footer" style="display:none; justify-content: space-between;">
				...
				<div class="pull-right">
					<button type="submit" class="btn btn-soft-facebook btn-smsq bold mb-0" name="upload">Okay</button>
				</div>
			</div>
			<?= $this->Form->end() ?>
			<!-- </form> -->
...
<div id="review_photo_success_modal" class="js-modal-window u-modal-window" style="width: 620px;">
	<div class="card mb-9">
		<!-- Header -->
		<header class="card-header bg-light py-3 px-5">
...
<script>
	...
	function getUploadedPhotosData() {
		var valid = true;
	...
	function uploadReviewPhotos(business_review_id) {
		...
		if (!valid) {
			...
			$.ajax({
				...
				url: "<?= $this->Url->build(['prefix' => false, 'controller' => 'Api', 'action' => 'uploadReviewPhotos']); ?>",
				data: {
					photos: JSON.stringify(sendData),
					business_review_id: business_review_id,
					photo_mode: "<?= !empty($photo_mode) ? $photo_mode : '' ?>"
				},
				cache: false,
	...
	$(document).ready(function() {
		var count = 0;
		rev_succes_modal = new Custombox.modal({
			content: {
				effect: 'fadein',
				target: '#review_photo_success_modal'
			}
		});
