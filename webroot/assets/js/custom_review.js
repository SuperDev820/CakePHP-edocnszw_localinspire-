...
function load_review_for_business(page, top_move){
	console.log(review_filter);
	// $('#reviews_for_business').css('display', 'none');
	var $scrollTo = $('#pills-one-tab');
	$.ajax({
		type: "POST",
		url: base_url + "v/get_reviews_for_business",
		data:{
			page: page,
			bid: bid,
		...
		success: function (data) {
			...
			$('#reviews_for_business').html(data);
			$("html, body").animate({ scrollTop: scroll_top }, "slow");
			// $('#reviews_for_business').css('display', 'block');
...
function load_question_answer(page, top_move){
	var $scrollTo = $('#pills-two-tab');
	$.ajax({
		type: "POST",
		url: base_url + "v/get_qa_for_business",
		data:{
			page: page,
			bid: bid,
		},
...
function load_questions(page, top_move){
	var $scrollTo = $('#questions');
	$.ajax({
		type: "POST",
		url: base_url + "v/get_ajax_questions",
		data:{
			page: page,
			bid: bid,
		...
		success: function (data) {
			...
			$("html, body").animate({ scrollTop: scroll_top }, "slow");
			//*** start pagination ***//
			$.SRCore.components.SRUnfold.init($('#questions [data-unfold-target]'));
			// return true;
...
function load_single_question(page, top_move){
	var $scrollTo = $('#answer_list');
	$.ajax({
		type: "POST",
		url: base_url + "v/get_ajax_single_question",
		data:{
			page: page,
		...
		success: function (data) {
			...
			$("html, body").animate({ scrollTop: scroll_top }, "slow");
			//*** start pagination ***//
			$.SRCore.components.SRUnfold.init($('#answer_list [data-unfold-target]'));
			// return true;
...
function review_pagemove(page, top_move = true){
	current_page = page;
	load_review_for_business(page, top_move);
	// $('#review_pagination').pagination('drawPage', $(this).data('page'));
}

function qa_pagemove(page, top_move = true){
	q_current_page = page;
	load_question_answer(page, top_move);
}
function tips_pagemove(page, top_move = true){
	a_current_page = page;
	load_tips(page, top_move);
}

function question_pagemove(page, top_move = true){
	question_page = page;
	load_questions(page, top_move);
}
function single_question_pagemove(page, top_move = true){
	single_question_page = page;
	load_single_question(page, top_move);
}

$(document).on('ready', function () {
	review_counts = $('#review_counts').text().trim();
	question_counts = $('#question_counts').text().trim();
	advice_counts = $('#advice_counts').text().trim();
	bid = $('input[id=business]').val();
	console.log(bid);
	...
	// $('#reviews_for_business').on('click', '*[data-modal-target="#reportreviewModal"]', function(e){
	// 	modal.open();
	// })
	...
	$('.review_filter input[type=checkbox]').change(function(){
		// console.log()
		review_filter = $('.review_filter input:checked').map(function(i, e) {return e.value}).toArray();
		current_page = 1;
